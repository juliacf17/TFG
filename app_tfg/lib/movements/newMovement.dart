...
class NuevaVentaScreen extends StatefulWidget {
  final bool isVenta; // Variable to determine if it is a sale or a loan
...
class _NuevaVentaScreenState extends State<NuevaVentaScreen> {
  ...
  void _accept() {
    if (widget.isVenta && _selectedPaymentMethod == null) {
      // Show an alert if the payment method is not selected for a sale
      showDialog(
        context: context,
        builder: (BuildContext context) {
          return AlertDialog(
            title: Text("Método de pago requerido"),
            content: Text("Debes seleccionar un método de pago."),
            actions: [
              TextButton(
                child: Text("OK"),
                onPressed: () {
    ...
    if (!widget.isVenta && _selectedClientId == 0) {
      // Show an alert if the client is not selected for a loan
      showDialog(
        context: context,
        builder: (BuildContext context) {
          return AlertDialog(
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text(widget.isVenta ? 'Nueva Venta' : 'Nuevo Préstamo',
