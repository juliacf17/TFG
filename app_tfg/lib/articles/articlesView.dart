...
class _ArticleScreenState extends State<ArticleScreen> {
                                                ...
                                                showDialog(
                                                  ...
                                                  builder:
                                                    ...
                                                    return AlertDialog(
                                                      ...
                                                      content: Text(
                                                        "No se puede eliminar un artículo asignado a un movimiento.",
                                                      ),
                                                      actions: [
                                                        TextButton(
                                                          child: Text(
  ...
  Future<bool> _deleteArticle(String articleId) async {
    try {
      // Obtener los IDs de las tallas asociadas al artículo
      final tallasResponse =
          await client.from('tallas').select('id').eq('articuloId', articleId);

      // Extraer los IDs de las tallas
      List<int> tallaIds =
          (tallasResponse as List).map((talla) => talla['id'] as int).toList();

      // Verificar si alguna de estas tallas está vinculada a algún movimiento
      final movimientosResponse = await client
          .from('articulosMov')
          .select('id')
          .inFilter('tallasId', tallaIds);

      // Si hay movimientos vinculados a alguna talla del artículo, no se elimina
      if (movimientosResponse.isNotEmpty) {
        return false;
      }
