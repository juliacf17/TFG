...
class _NewArticleState extends State<NewArticle> {
  ...
  Future<bool> _addTallasToDatabase() async {
    try {
      final articleId =
          await _addArticleToDatabase(); // Espera a que se complete la inserción del artículo
...
class _NewArticleState extends State<NewArticle> {
  ...
  Future<bool> _addArticleToDatabase() async {
    try {
      ...
      final response = await client.from('articulos').insert(newArticle);

      // Obtener el ID del artículo insertado
      final int articleId = response['id'];

      // Insertar tallas, colores y cantidades en la tabla 'tallas'
      for (int i = 0; i < sizeControllers.length; i++) {
        await client.from('tallas').insert({
          'articuloId': articleId,
          'talla': sizeControllers[i].text.trim(),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Artículo añadido exitosamente')),
      );
